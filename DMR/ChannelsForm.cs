...
namespace DMR
{
    public class ChannelsForm : DockContent, IDisp, ISingleRow
    {
            ...
            try
            {
                ...
                for (int i = 0; i < ChannelForm.data.Count; i++)
                {
                    if (ChannelForm.data.DataIsValid(i))
                    {
                        ...
                        int index = this.dgvChannels.Rows.Add((i + 1).ToString(), channelOne.Name, channelOne.ChModeS, channelOne.RxFreq, channelOne.TxFreq, channelOne.TxColor.ToString(), channelOne.RepeaterSlotS, channelOne.ContactString, channelOne.RxGroupListString, channelOne.LibreDMR_PowerStr, channelOne.RxTone, channelOne.TxTone);//, );
                ...
                using (CsvFileWriter csvFileWriter = new CsvFileWriter(new FileStream(saveFileDialog.FileName, FileMode.Create), Encoding.Default))
                {
                    ...
                    foreach (DataGridViewRow row in rows)
                    {
                        ...
                        if (ChannelForm.data.DataIsValid(i))
                        {
                            ...
                            csvRow.Add(channelOne.LibreDMR_PowerStr);
                            csvRow.Add(channelOne.BandwidthString);
                            csvRow.Add(channelOne.OnlyRxString);
                            csvRow.Add(channelOne.SquelchLevelStr);
                    ...
                    if (csvRow.SequenceEqual(SZ_EXPORT_HEADER_TEXT))
                    {
						...
						*/
                        ...
                        while (csvFileReader.ReadRow(csvRow))
                        {
                            ...
                            if (contactName == Settings.SZ_NA || Int32.Parse(callId) == 0)
                            {
                                value.Contact = 0;
                            }
                            else
                            ...
                            value.LibreDMR_PowerStr = ((List<string>)csvRow)[num++];
                            value.BandwidthString = ((List<string>)csvRow)[num++];
                            value.OnlyRxString = ((List<string>)csvRow)[num++];
                            value.SquelchLevelStr = ((List<string>)csvRow)[num++];
        ...
        public void RefreshSingleRow(int index)
        {
            ...
            this.dgvChannels.Rows[index2].Cells[5].Value = channelOne.LibreDMR_PowerStr;
        ...
        private void handleInsertClick()
        {
            ...
            this.dgvChannels.Rows.Insert(minIndex, (minIndex + 1).ToString(), channelOne.Name, channelOne.RxFreq, channelOne.TxFreq, channelOne.ChModeS, channelOne.LibreDMR_PowerStr, channelOne.RxTone, channelOne.TxTone, channelOne.TxColor.ToString(), channelOne.RxGroupListString, channelOne.ContactString, channelOne.RepeaterSlotS);
